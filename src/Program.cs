        ...
        static void Main(string[] args) {
            ...
            if (args.Length > 0 && args[0] == "auto") {
                if (File.Exists(cfgPath)) {
                    var list = JsonSerializer.Deserialize<List<Account>>(File.ReadAllText(cfgPath));
                    if (list != null) foreach (var a in list) {
                        if (string.IsNullOrEmpty(a.Url)) continue;
                        string target = string.IsNullOrEmpty(a.Drive) ? "*" : (a.Drive.EndsWith(":") ? a.Drive : a.Drive + ":");
                        ProcessStartInfo psi = new ProcessStartInfo("net.exe", $"use {target} {a.Url} /user:{a.User} {a.Pass} /persistent:no") { WindowStyle = ProcessWindowStyle.Hidden, CreateNoWindow = true, UseShellExecute = false };
                        Process.Start(psi);
                    }
                }
                return;
            ...
            dgv.CellFormatting += (s, e) => { if (e.ColumnIndex == 3 && e.Value != null) { e.Value = new string('*', e.Value.ToString()?.Length ?? 0); e.FormattingApplied = true; } };
            ...
            btnSave.Click += (s, e) => {
                ...
                foreach (DataGridViewRow row in dgv.Rows) {
                    ...
                    list.Add(new Account { 
                        ...
                        Drive = row.Cells[4].Value?.ToString() ?? "" 
                    });
                }
                File.WriteAllText(cfgPath, JsonSerializer.Serialize(list, new JsonSerializerOptions { WriteIndented = true }));
